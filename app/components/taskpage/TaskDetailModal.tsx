...
export default function TaskDetailModal({ isOpen, onClose, task }: ModalProps) {
    ...
    const handleMarkAsDone = async () => {
        ...
        const confirmDone = confirm("Tandai tugas ini sebagai selesai dan pindahkan ke arsip?");
        ...
        const { error } = await supabase
            .from('assignments')
            .update({ status: 'completed' }) // Ubah status jadi completed
            .eq('id', task.id);
        ...
        if (error) {
            ...
            // Sukses
            onClose(); // Tutup modal
            router.refresh(); // Refresh halaman agar tugas hilang dari list aktif
            window.location.reload(); // Paksa reload agar state parent benar-benar bersih
        }
        setIsLoading(false);
    };
                ...
                <div className="p-4 border-t border-gray-100 flex gap-3 shrink-0 bg-white">
                    ...
                    {isAdmin && (
                        <button
                            ...
                            disabled={isLoading}
                            className="flex-1 py-2.5 rounded-xl bg-[#D06E49] text-white font-medium hover:bg-[#b55c3b] transition-colors shadow-lg shadow-orange-200 disabled:opacity-50 flex justify-center items-center gap-2"
                        >
                            {isLoading ? (
                                <span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span>
                            ) : (
                                <>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M9 16.17L4.83 12l-1.42 1.41L9 19L21 7l-1.41-1.41z" /></svg>
                                    <span>Selesai</span>
                                </>
                            )}
                        </button>
